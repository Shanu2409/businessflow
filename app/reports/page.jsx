...
const ReportsPage = () => {
  ...
  const fetchReferenceData = useCallback(async () => {
    try {
      ...
      setAvailableBanks(
        Array.isArray(banksRes.data.data) ? banksRes.data.data : []
      );
      setAvailableWebsites(
        Array.isArray(websitesRes.data.data) ? websitesRes.data.data : []
      );
        ...
        case "userActivity":
          dataToExport = reportData.data.map((user) => ({
            ...
            firstActivity: user.firstActivity
              ? format(new Date(user.firstActivity), "yyyy-MM-dd HH:mm:ss")
              : "",
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
            ...
            <p className="text-2xl font-bold">
              {data.counts?.transactionCount || 0}
            </p>
          </div>
          <div className="bg-teal-50 p-4 rounded-lg border border-teal-200">
            <h3 className="text-sm font-medium text-teal-700 mb-2">Total ac</h3>
            <p className="text-2xl font-bold">
              {data.counts?.accountCount || 0}
            </p>
          </div>
        </div>
            ...
            <table className="w-full table-auto border-collapse">
              ...
              <tfoot className="bg-gray-100 font-bold">
                <tr>
                  ...
                  <td className="border border-gray-300 px-4 py-2 text-right">
                    {new Intl.NumberFormat("en-IN").format(
                      data.balances?.totalBankBalance || 0
                    )}
                  </td>
                </tr>
              </tfoot>
            ...
            <table className="w-full table-auto border-collapse">
              ...
              <tfoot className="bg-gray-100 font-bold">
                <tr>
                  ...
                  <td className="border border-gray-300 px-4 py-2 text-right">
                    {new Intl.NumberFormat("en-IN").format(
                      data.balances?.totalWebsiteBalance || 0
                    )}
                  </td>
                </tr>
              </tfoot>
  ...
  const renderTransactionSummary = () => {
    try {
      if (!reportData) {
        return (
          <div className="p-6 text-center">No transaction data available</div>
        );
      }
      ...
      transactionData.forEach((item) => {
        if (item) {
          ...
          summary.totalDeposits += Number(
            item.depositAmount || item.totalDeposits || 0
          );
          summary.totalWithdrawals += Number(
            item.withdrawAmount || item.totalWithdrawals || 0
          );
      ...
      return (
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-6">Flow Summary</h2>
              ...
              <div className="flex items-start">
                ...
                <div>
                  <p className="text-sm text-blue-700 font-medium">
                    Time-based filtering active
                  </p>
                  <p className="text-xs text-blue-600">
                    Showing transactions between {filters.startTime} and{" "}
                    {filters.endTime}
                  </p>
                </div>
              </div>
            ...
            <div>
              <div className="grid grid-cols-1 gap-4 mb-6">
                ...
                <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {summary.depositCount || 0} transactions
                  </p>
                </div>
                <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {summary.withdrawalCount || 0} transactions
                  </p>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  {displayItems && displayItems.length > 0 ? (
                    displayItems.map((tx, index) => {
                      const depositAmt =
                        tx?.depositAmount || tx?.totalDeposits || 0;
                      const withdrawAmt =
                        tx?.withdrawAmount || tx?.totalWithdrawals || 0;
                      return (
                        <tr
                          key={index}
                          className={
                        ...
                        >
                          ...
                          <td className="border border-gray-300 px-4 py-2 text-right text-green-600">
                            {new Intl.NumberFormat("en-IN").format(depositAmt)}
                          </td>
                          <td className="border border-gray-300 px-4 py-2 text-right text-red-600">
                            {new Intl.NumberFormat("en-IN").format(withdrawAmt)}
                          </td>
                          ...
                          <td className="border border-gray-300 px-4 py-2 text-right">
                            {tx?.count ||
                              tx.depositCount + tx.withdrawalCount ||
                              0}
                          </td>
                        </tr>
                      );
  ...
  const renderBalanceSummary = () => {
      ...
      datasets: [
        {
          label: "Bank Balances",
          data: data.banks
            ? data.banks.map((bank) => bank.current_balance)
            : [],
          backgroundColor: [
            "rgba(54, 162, 235, 0.6)",
            "rgba(75, 192, 192, 0.6)",
    ...
    const websiteChartData = {
      labels: data.websites
        ? data.websites.map((site) => site.website_name)
        : [],
      datasets: [
        {
          label: "Website Balances",
          data: data.websites
            ? data.websites.map((site) => site.current_balance)
            : [],
          backgroundColor: [
            "rgba(255, 159, 64, 0.6)",
            "rgba(255, 99, 132, 0.6)",
      ...
      datasets: [
        {
          ...
          backgroundColor: [colors.bank, colors.website, colors.net],
          borderColor: [
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(totalBankBalance)}
            </p>
          </div>
          <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
            <h4 className="text-sm font-medium text-amber-700 mb-2">
              Total Website Balance
            </h4>
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(totalWebsiteBalance)}
            </p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <h4 className="text-sm font-medium text-purple-700 mb-2">
            ...
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(netBalance)}
            </p>
          </div>
        </div>
  ...
  const renderUserActivity = () => {
    if (!reportData || !reportData.data) {
      return (
        <div className="p-6 text-center">No user activity data available</div>
      );
    }
        ...
        {(filters.startTime !== "00:00" || filters.endTime !== "23:59") && (
          <div className="bg-blue-50 p-3 rounded-md border border-blue-200 mb-4">
            <div className="flex items-start">
              ...
              <div>
                <p className="text-sm text-blue-700 font-medium">
                  Time filter active: {filters.startTime} - {filters.endTime}
                </p>
                <p className="text-xs text-blue-600">
                  First and last activity times now show precise timestamps
                </p>
              </div>
            </div>
          </div>
          ...
          <table className="w-full table-auto border-collapse">
              ...
              {data.length > 0 ? (
                data.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      {user.firstActivity
                        ? formatDateWithPossibleTime(null, user.firstActivity)
                        : "-"}
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      {user.lastActivity
                        ? formatDateWithPossibleTime(null, user.lastActivity)
                        : "-"}
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-center">
            ...
            <tfoot className="bg-gray-100 font-bold">
              <tr>
                ...
                <td className="border border-gray-300 px-4 py-2 text-center">
                  {data.reduce(
                    (sum, user) => sum + (user.totalTransactions || 0),
                    0
                  )}
                </td>
                <td className="border border-gray-300 px-4 py-2 text-right">
                  {new Intl.NumberFormat("en-IN", {
                    ...
                    data.reduce((sum, user) => sum + (user.totalAmount || 0), 0)
                  )}
                </td>
                <td
  ...
  const renderTrendAnalysis = () => {
    ...
    const chartData = {
      labels: data.map(
        (item) =>
          formatDateWithPossibleTime(item.date, item.createdAt) ||
          item.label ||
          ""
      ),
      datasets: [
        ...
        },
        {
          label: "Net Flow",
          data: data.map(
            (item) => (item.depositAmount || 0) - (item.withdrawAmount || 0)
          ),
    ...
    return (
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold mb-6">Flow Trend Analysis</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-green-50 p-4 rounded-lg border border-green-200">
            ...
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(
                data.reduce((sum, item) => sum + (item.depositAmount || 0), 0)
              )}
            </p>
          </div>
          <div className="bg-red-50 p-4 rounded-lg border border-red-200">
            ...
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(
                data.reduce((sum, item) => sum + (item.withdrawAmount || 0), 0)
              )}
            </p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            ...
            <p className="text-xl font-bold">
              {new Intl.NumberFormat("en-IN").format(
                data.reduce(
                  (sum, item) =>
                    sum +
                    ((item.depositAmount || 0) - (item.withdrawAmount || 0)),
                  0
                )
              )}
            </p>
              ...
              {data.map((item, index) => {
                ...
                if (index > 0) {
                  const currentNet =
                    (item.depositAmount || 0) - (item.withdrawAmount || 0);
                  const prevNet =
                    (data[index - 1].depositAmount || 0) -
                    (data[index - 1].withdrawAmount || 0);
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-xs text-blue-700 font-medium">
                        Time filtering is now available!
                      </p>
                      <p className="text-xs text-blue-600 mt-1">
                        Select both date and time for precise transaction
                        filtering. Particularly helpful for high-volume
                        transaction periods.
                      </p>
                    </div>
                  </div>
